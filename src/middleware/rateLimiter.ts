...
const WINDOW_SIZE:number=60;
const MAX_REQUESTS:number=10;

const rateLimiter=async(req:Request,res:Response,next:NextFunction)=>{

    const clientIP=req.ip || req.socket.remoteAddress;

    const key:string=`ratelimit:${clientIP}`;

    try{
        const rawCount = await redis.get(key);
const currentCount: number = rawCount === null ? 0 : Number(rawCount);
...
if (currentCount === 0) {
    ...
    next();
            ...
            const ttl:number | null=await redis.ttl(key);
